...
def indexar_rosto_da_imagem_s3(s3_path: str, external_image_id: str, detection_attributes: list[str] | str = "DEFAULT") -> dict | None:
    ...
    try:
        ...
        logger.error(f"❌ Erro inesperado ao indexar rosto para '{external_image_id}': {e}", exc_info=True)
        return None
...
def reconhecer_aluno_por_bytes(image_bytes: bytes, face_match_threshold: int = 80) -> str | None:
    ...
    try:
        logger.info(f"Tentando reconhecer aluno a partir de imagem em memória ({len(image_bytes)} bytes) com threshold de {face_match_threshold}%.")
