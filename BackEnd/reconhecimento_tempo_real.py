...
TEMPO_ESPERA = (
    10  # Tempo em segundos para evitar reconhecer o mesmo rosto repetidamente
)
...
def verificar_setup_aws_rekognition(
    client_rek, collection_id_to_check, region_name_config
):
    """
    ...
    if not client_rek:
        logger.info(
            "Cliente Rekognition não inicializado. Verificação de setup cancelada."
        )
        return False
    ...
    try:
        ...
        logger.info(
            "Tentando listar coleções para testar a conexão e permissões básicas..."
        )
        response_list = client_rek.list_collections()

        logger.info(
            f"Conexão bem-sucedida! Coleções Rekognition existentes: {response_list.get('CollectionIds', [])}"
        )
        ...
        if error_code == "UnrecognizedClientException":
            ...
            logger.error(
                "Causa provável: O usuário IAM não tem as permissões necessárias."
            )
            logger.error(
            ...
            error_code == "InvalidSignatureException"
            ...
            logger.error(
                "Causa provável: Muitas requisições para a AWS. Tente novamente mais tarde."
            )

        elif (
            error_code == "InvalidParameterException"
            and "Unable to parse region" in error_message
        ):
            logger.error(
            ...
            logger.error(
                f"Um erro inesperado do cliente AWS ocorreu durante a verificação: {error_message}"
            )
        ...
        Exception
    ) as e:  # Captura outras exceções (ex: BotoCoreError para problemas de endpoint/rede)
        logger.error(f"ERRO GERAL durante a verificação do setup AWS: {e}")
        logger.error(
            "Verifique sua conexão de rede e as configurações de endpoint da AWS, se aplicável."
        )
...
def processar_rekognition():
    ...
    if not rekognition_client:  # Adiciona verificação
        logger.error(
            "Cliente Rekognition não está disponível na thread processar_rekognition."
        )
    ...
    while rodando:
        ...
        frame_para_processar = (
            frame_atual  # Copia para evitar problemas de concorrência
        )
        ...
        try:
            ...
            if response["FaceMatches"]:
                ...
                for match in response["FaceMatches"]:
                    ...
                    if (
                        agora - faceid_ultimo_reconhecimento.get(face_id, 0)
                        < TEMPO_ESPERA
                    ):
                        continue
            ...
            logger.info(
                "Nenhum rosto detectado no frame pelo Rekognition. Continuando..."
            )
...
def reconhecer_em_tempo_real():
    ...
    if cap.isOpened():
        ...
        logger.error(
            "Erro ao abrir a câmera. Verifique se está conectada e não em uso por outro programa."
        )
        return
    ...
    while True:
        ...
        frame_atual = (
            frame.copy()
        )  # Garante que a thread de rekognition use uma cópia estável
        ...
        time.sleep(
            0.05
        )  # Ajuste conforme necessário para equilibrar responsividade e uso de CPU
...
if __name__ == "__main__":
    ...
    if not rekognition_client:  # Verifica se o cliente global foi carregado
        logger.error(
            "Cliente Rekognition não pôde ser inicializado. Verifique aws_clients.py e as configurações."
        )
        sys.exit(1)

    # A função verificar_setup_aws_rekognition agora usa o rekognition_client importado
    if not verificar_setup_aws_rekognition(
        rekognition_client, COLLECTION_ID, AWS_REGION
    ):
        logger.error(
            "Falha crítica na verificação do setup da AWS. A aplicação será encerrada."
        )
